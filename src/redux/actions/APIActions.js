...
export const fetchProducts = () => async (dispatch) => {
  try {
    const response = await axiosClient.get('/admin/product/');
    dispatch({
...
export const fetchProductsById = (id) => async (dispatch) => {
  console.log(id);
    ...
    try {
      const response = await axiosClient.get(`/admin/product/${id}`);
      dispatch({
...
export const fetchCategoriesById = (id) => async (dispatch) => {
    try {
      const response = await axiosClient.get(`/admin/category/${id}`);
      dispatch({
...
export const fetchCartProductByIds = (products) => async (dispatch) => {
  try {
    ...
    const response = await axiosClient.post('/cart/products', productData);    
    dispatch({
...
export const updateCartProductQuantity = (userId, productId, newQuantity) => async (dispatch) => {
  try {
    await axiosClient.patch(`/cart/product-update/${userId}`, { productId: productId, quantity: newQuantity });
    dispatch({
...
export const removeCartProduct = (userId, productId) => async (dispatch) => {
  try {
    await axiosClient.patch(`/cart/product/${userId}`, { productId: productId });
    dispatch({
...
export const addToWishlist = (userId, productId) => async (dispatch) => {
  try {
    const { data } = await axiosClient.post(`/wishlist/add/${userId}`, { productId });
    dispatch({
...
export const removeFromWishlist = (userId, productId) => async (dispatch) => {
  try {
    const { data } = await axiosClient.post(`/wishlist/remove/${userId}`, { productId });
    dispatch({
...
export const getOrderByOrderId = (orderId) => async (dispatch) => {    
  try {
    const { data } = await axiosClient.get(`/order/id/${orderId}`);
    dispatch({
...
export const getOrderByUserId = (userId) => async (dispatch) => {  
  try {
    const { data } = await axiosClient.get(`/order/${userId}`);
    dispatch({
...
export const saveOrder = (userId, products, shipping, total) => async (dispatch) => {  
  try {
    const { data } = await axiosClient.post(`/order/${userId}`, { products, shipping, total });
    dispatch({
      type: SAVE_ORDER,
      payload: data,
    });
